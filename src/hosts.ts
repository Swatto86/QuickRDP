...
function isValidServerName(server: string, domain: string): boolean {
  ...
  if (!serverLower.endsWith(expectedSuffix)) return false;
  
  // Check if server has a valid hostname prefix
  const prefix = serverLower.slice(0, -expectedSuffix.length); // Remove ".domain" suffix
  
  // Prefix must not be empty and must be a valid hostname
  if (prefix.length === 0) return false;
  
  // Validate the prefix (can be a single hostname or multiple parts like "dc01" or "dc01.subdomain")
